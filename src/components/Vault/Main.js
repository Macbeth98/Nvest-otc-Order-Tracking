...
export default class Main extends Component {
  ...
  async componentWillMount(){
    ...
    var res = await axios.get(fetch.url+`vault/get_txn?id=${this.props.match.params.txn_id}`);
    //console.log(res.data);
    var txn = res.data;
    if(txn){
    await this.setState({
      name: txn.name?txn.name: "",
      pid: txn.pid,
      amt_received: txn.amt_received?txn.amt_received: 0,
      quote: txn.amt_credit,
  ...
  render() {
    return (
      <div className="mb-5">
        {this.state &&  this.state.pid.length>0?(<div>
       <SectionOne name={this.state.name}  />
       <SectionTwo  />
       <Checkout/>
